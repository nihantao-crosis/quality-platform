...
## AQL 接收数可选优先数近似(v1.27.0,v1.28.0 纠正表述)

AQL 页加一个开关**「接收数 Ac:优先数近似 / 二项近似」**:

- **优先数近似(默认)**:把二项接收数**吸附到优先数列**`0,1,2,3,5,7,10,14,21,30,44…`(取最小满足
  95% 接收概率的优先数)。
- **二项近似**:取最小满足 95% 接收概率的接收数(原算法)。

⚠ **诚实纠正(v1.28.0)**:v1.27.0 曾把此模式称为"国标数列"并宣称"与国标已知格全部相符",
**这是错的**——外部审核(Codex/Grok)逐格对拍 GB/T 2828.1 表 2-A 后确认:本模式**不是逐格查表**,
只有部分格巧合一致(J/2.5=5、L/2.5=10、M/2.5=14),另有一批格与真表不符
(F/1.0 近似给 1、真表为 0;N/4.0 近似给 30、真表为 21;G/2.5 近似给 2、真表为 3),
且未实现"箭头改档"。**真正的国标主表查表待表 2-A 原图逐格转录后落地**。
