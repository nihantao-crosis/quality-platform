...
- **数据**：CSV/剪贴板导入（变量矩阵 n=2..10 / P 图不良数 / C 图缺陷数），文本列
  保留为分组列；**手工录入**（双击单元格编辑、添加子组、悬停行号删行,编辑演示集
  自动转「副本」）；localStorage 持久化 + 最近数据集（项目管理器点击加载）；MES 模拟
  采集流（实时追加子组,SPC 滚动更新,架构按 `connectMes()` 预留串口/OPC-UA）。
...
- **ANOVA / Gage R&R**：引擎实算（F 分布 CDF/不完全贝塔）;导入数据含分组列时
  按所选 响应×分组 / 测量×部件×操作员 列实算,否则回落演示研究。
- **AQL**：方案实时重算 + OC 曲线 + **正常/加严/放宽转移规则模拟**（简化模型）。
- **导出**：Excel 为**真 .xlsx**（SheetJS,数据/统计摘要/失控点三 sheet）;PDF 为
  **图文 HTML 报告**（内嵌 X̄ 控制图/直方图/概率图 SVG + 统计表,打开后打印即得
  PDF）;PPT/Word 为文本报告;Web 端支持浏览器打印当前页。
- **工程**：ESLint + UI 冒烟测试(69 项测试);`ci.yml` 每次 push 质量门;
