...
## AQL 接收数可选国标优先数列(v1.27.0)

AQL 页再加一个开关**「接收数 Ac:国标数列 / 二项近似」**,让接收数也向国标主表看齐:

- **国标数列(默认,近似主表)**:GB/T 2828.1 主表的接收数取自"优先数列"`0,1,2,3,5,7,10,14,21,30,44…`
  (不是任意整数)。本模式把 Ac **吸附到优先数列**——取最小满足 95% 接收概率的优先数。
  已核对与国标已知格全部相符(F/1.0=1、J/2.5=5、L/2.5=10、M/2.5=14、N/4.0=30 等),
  差异集中在大样本档(二项会给 9/13/27 这类非标数,国标吸附为 10/14/30)。
- **二项近似**:取最小满足 95% 接收概率的接收数(原算法)。
- 诚实标注:这是**近似主表**——接收数与字码/样本量已对齐国标,但"小样本+小 AQL 的箭头改档"
  (↑↓ 改用相邻方案)尚未实现,待拿到原表逐格校准。字码与样本量 n 已走真国标查表(v1.26.0)。
