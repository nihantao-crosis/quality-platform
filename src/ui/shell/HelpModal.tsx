...
const TOPICS: Topic[] = [
  ...
  },
  {
    ...
    lines: [
      'GB/T 2828.1:批量+检验水平 → 字码 → 样本量 n。默认「国标主表」按表 2-A 逐格查 Ac(含 ↑↓ 箭头改档)。',
      '注意:真表小样本 Ac=0 方案的 Pa(AQL) 可低至约 88%(α≈12%),并非恒 ≥95%;仅可选的「二项近似」法才按最小满足 Pa(AQL) ≥ 95% 取 Ac。',
      '样本量 n ≥ 批量 N 时按国标转 100% 全检(逐件检验)。',
      'OC 曲线:Pa(p) = BinomCDF(Ac, n, p);RQL 为 Pa = 10% 的批质量。',
      '转移规则:正常⇄加严⇄放宽,依据连续批的接收/拒收记录(见 AQL 页模拟器,为教学简化模型,非正式表 2-B/2-C)。',
