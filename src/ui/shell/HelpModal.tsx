...
const TOPICS: Topic[] = [
  ...
  },
  {
    ...
    lines: [
      'GB/T 2828.1-2012:批量+检验水平 → 表 1 初始字码 → 正常/加严/放宽分别查表 2-A/2-B/2-C(含 ↑↓ 箭头改档)。',
      '正式主表的小样本 Ac=0 方案在 AQL 点的接收概率可低至约 88%(α≈12%)，不能用“Pa(AQL)恒≥95%”的二项近似口径解释。',
      '样本量 n ≥ 批量 N 时按国标转 100% 全检(逐件检验)。',
      '录入样本(或全检)实际不合格品数 d 后，系统按 d≤Ac / d≥Re 自动判定，并留存批次号、检验人、方案和初检/复验追溯记录。',
      'OC 曲线:Pa(p) = BinomCDF(Ac, n, p);RQL 为 Pa = 10% 的批质量。',
      '转移规则按第 9.3/9.4 条执行：2/5 批不接收转加严；转移得分≥30+生产稳定+负责部门同意才放宽；加严累计 5 批不接收时暂停抽样检验。',
