...
export function Toolbar() {
  ...
  const btns: Array<{ label: string; icon: IconName; onClick: () => void }> = [
    ...
    { label: '保存', icon: 'check', onClick: () => { import('../../platform/project').then((m) => m.exportProject(__APP_VERSION__, useApp.getState().projectName).then((dest) => { if (dest) showToast('项目已保存: ' + dest); })); } },
