...
describe('自查F2:legacy 导出在哨兵状态下能力章节写「未运行」,不整份失败', () => {
  it('textReport 规格量纲误配 → 不抛异常,含未运行说明,其余章节仍在', () => {
    ...
    const text = textReport(M, { lsl: 500, tgt: 900, usl: 1400 }, assessSpcCharts(M, DEFAULT_SPC_RULES, DEFAULT_RULE_K));
  ...
  it('仅 R 图失控时通用报告先报变异未受控，不再同时写「过程受控」', () => {
    ...
    const text = textReport(useData.getState().model, { lsl: 0, tgt: 10, usl: 20 }, assessSpcCharts(useData.getState().model, DEFAULT_SPC_RULES, DEFAULT_RULE_K));
  ...
  it('仅 MR 图存在模式异常时也不会把 I 图无异常写成整体受控', () => {
    ...
    const text = textReport(useData.getState().model, { lsl: -10, tgt: 0, usl: 10 }, assessSpcCharts(useData.getState().model, DEFAULT_SPC_RULES, DEFAULT_RULE_K));
