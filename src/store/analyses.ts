...
import {
  ...
  type InspectionLevel, type AnovaMode, type AqlMethod, type AqlAcMethod,
} from '../core';
...
function buildSummary(kind: AnalysisKind): Omit<SavedAnalysis, 'id' | 'createdAt' | 'datasetName' | 'snapshot'> | null {
  ...
  switch (kind) {
    ...
    case 'aql': {
      const plan = aqlPlan(app.aqlLot, app.aqlLevel, app.aqlAQL, app.aqlMethod, app.aqlAcMethod);
      return {
...
function snapshotOf(kind: AnalysisKind): AnalysisSnapshot {
  ...
  if (kind === 'aql') { snap.aqlLot = a.aqlLot; snap.aqlLevel = a.aqlLevel; snap.aqlAQL = a.aqlAQL; snap.aqlMethod = a.aqlMethod; snap.aqlAcMethod = a.aqlAcMethod; }
