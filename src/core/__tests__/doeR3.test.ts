...
describe('generateFractionalFactorialDesign:2^(4-1) D=ABC 设计矩阵逐行锚点(Yates 序)', () => {
  ...
  it('全折叠 → 16 行,追加行全部取反(含 D 列 = −原 D),折叠块记为区组 2', () => {
    const folded = generateFractionalFactorialDesign([pm1('A'), pm1('B'), pm1('C'), pm1('D')], 1, { fold: 'full' });
    ...
    for (let i = 0; i < 8; i++) {
      ...
      expect(folded.rows[8 + i].slice(3, 7)).toEqual(d.rows[i].slice(3, 7).map((v) => -v));
