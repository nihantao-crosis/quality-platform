 ...
 * 前 16 档(0.010–10)为「百分不合格品」体系(二项,d ≤ n);
 * 后 10 档(15–1000)为「每百单位不合格数」体系(泊松计数,d 可大于 n)。 */
...
/** AQL 体系:≤10 为百分不合格品(二项);≥15 为每百单位不合格数(泊松,d 允许大于 n)。 */
export type AqlRegime = 'percent' | 'per100';

/** 按 GB/T 2828.1 §5.2 判定 AQL 所属体系(两体系在 10 与 15 之间断开,无中间档)。 */
...
/** 生产方风险 α = 1 − Pa(AQL)，百分数。体系由 AQL 值本身判定(≥15 为 per100/泊松)。 */
export function producerRiskPct(plan: SamplingPlan, aqlPct: number): number {
  const regime = aqlRegime(aqlPct);
