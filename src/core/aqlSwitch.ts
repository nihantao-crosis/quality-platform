 ...
 */
import { aqlPlan, acceptNumber, LETTERS, N_BY_CODE, type InspectionLevel, type SamplingPlan } from './aql';
...
/** 三种状态下的抽样方案（简化模型） */
export function plansByState(lot: number, level: InspectionLevel, aql: number): Record<InspState, SamplingPlan> {
  const normal = aqlPlan(lot, level, aql);
  const tightened: SamplingPlan = { ...normal, ac: Math.max(0, normal.ac - 1), re: Math.max(1, normal.ac) };
  // 放宽：字码降两档
  const idx = LETTERS.findIndex((l) => l[0] === normal.code);
  const rIdx = Math.max(0, idx - 2);
  const rCode = LETTERS[rIdx][0];
